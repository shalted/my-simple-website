...
```text
打开 Home：栈 [Home]，活动 {Home}
打开 Inventory：栈 [Home, Inventory]，活动 {Home, Inventory}
打开 ItemDetail：栈 [Home, Inventory, ItemDetail]
关闭 ItemDetail：栈回到 [Home, Inventory]，Detail 面板进入池
再次打开另一件物品详情：从池复用 Detail，并绑定新的 ItemId
```

复用时必须清理旧事件订阅、异步请求和 ItemId。否则视觉上是新详情页，回调仍可能操作上一件物品。
