...
编译器从静态类型知道“允许调用 Damage”，运行时再根据动态类型选择 Warrior 的实现。若对象被按值复制成 `Character`，派生部分被切掉，动态类型也就不再是 Warrior。

## 常见误解

- 有虚函数不等于标准规定对象第一个字段必须是虚表指针，那属于 ABI 实现。
- 基类指针指向派生对象，不代表按值复制基类也能保留派生部分。
- 虚析构解决通过基类指针删除派生对象的问题，不是让所有析构自动变快或线程安全。
