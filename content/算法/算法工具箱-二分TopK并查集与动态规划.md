...
```text
二分：[2, 5, 8, 12, 20] 中找 12 -> 先看 8，再只查右半
Top K：分数 [7, 2, 9, 5] 取前 2 -> [9, 7]
并查集：连接 (1,2)、(2,3) -> 1、2、3 属于同组
动态规划：爬 5 阶，每次 1 或 2 阶 -> ways(5)=8
```

同一个项目可能同时使用四种工具，但每一种都应对应一个明确的不变量或递推关系，不能因为“复杂度看起来更好”就机械套用。
