...
```text
初始攻击力：100，dirty = false
装备加成改为 +20：dirty = true
Buff 加成改为 +30：dirty 仍为 true
天赋加成改为 +10：dirty 仍为 true
帧末读取攻击力：重算 100 + 20 + 30 + 10 = 160，dirty = false
```

没有脏标记时可能重算三次；有脏标记后只在真正需要结果时算一次。若修改后立刻读取，就必须先 `EnsureFresh`，不能返回旧的缓存值。
