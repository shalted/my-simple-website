...
```text
初始 0.0 秒：释放技能，冷却结束时间设为 5.0
2.0 秒：剩余冷却 3.0，技能不可用
5.0 秒：冷却完成；延迟任务若设为 5 秒，也在此时触发一次
每 0.5 秒 Tick：0.5、1.0、1.5……共执行 10 次
```

帧时间若从 1.4 秒直接跳到 2.1 秒，按“是否补 Tick”的规则，可能只执行一次，也可能补执行 1.5 和 2.0 两次。这个选择必须由玩法语义决定。
